...
for k, adaptativo in enumerate(adaptativos):
    ...
    for j in range(rondas):
        ...
        results.append(PPN.train([row[0] for row in DATOS], RESULTADOS_ESPERADOS, DATOS_VALIDACION, eta=eta, epochs=epochs,
                                 tamanio_muestra_batch=350, early_stopping_treshold=estop, print_epochs=False, adaptativo=adaptativo))
    ...
    if graficar:
        ...
        if adaptativo:
            plt.savefig('informe/graficos/ej2/adaptativos_si.png')
        else:
            plt.savefig('informe/graficos/ej2/adaptativos_no.png')
...
plt.ylabel('Error/Funcion Costo')
plt.legend(loc=1)
plt.savefig('informe/graficos/ej2/adaptativos_promedios_entrenamiento.png')
plt.clf()
...
plt.ylabel('Error/Funcion Costo')
plt.legend(loc=1)
plt.savefig('informe/graficos/ej2/adaptativos_promedios_validacion.png')
plt.clf()
